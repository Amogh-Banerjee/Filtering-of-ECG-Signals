...
figure(4);
x_filtered = filter(cheby1_lpf, powerline_affected_ecg);

subplot(2, 1, 1);
plot(t, x_filtered, 'linewidth', 1);
set(gca, 'fontsize', 13, 'fontweight', 'bold');
title('Chebyshev 1 Signal with Powerline Interference filtered', 'fontsize', 15);
xlabel('Time (s)');
ylabel('Amplitude (mV)');
grid on;
...
figure(6);
x_filtered = filter(cheby1_hpf, baseline_affected_ecg);
